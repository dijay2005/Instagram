...
public class UserProfileAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
{
    ...
    private static final int USE_OPTIONS_ANIMATION_DELAY = 300;
    ...
    public UserProfileAdapter(Context context)
    {
        ...
        this.avatarSize = context.getResources().getDimensionPixelSize(R.dimen
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType)
    {
        if (TYPE_PROFILE_HEADER == viewType)
        {
            final View view = LayoutInflater.from(context).inflate(R.layout
                    .view_user_profile_header, parent, false);
        ...
        {
            final View view = LayoutInflater.from(context).inflate(R.layout
                    .view_user_profile_options, parent, false);
            StaggeredGridLayoutManager.LayoutParams layoutParams = (StaggeredGridLayoutManager
                    .LayoutParams) view.getLayoutParams();
            layoutParams.setFullSpan(true);
            view.setLayoutParams(layoutParams);
    ...
    {
        Picasso.with(context).load(profilePhoto).placeholder(R.drawable.img_circle_placeholder)
                .resize(avatarSize, avatarSize).centerCrop().transform(new CircleTransformation()
...

        {
            ...
            public boolean onPreDraw()
            {
                ...
                animationUserProfileHeader(holder);
                return false;
            }
        });
    }
    ...
    {
        Picasso.with(context).load(photos.get(position - MIN_ITEMS_COUNT)).resize(cellSize,
                cellSize).centerCrop().into(holder.ivPhoto, new Callback()
        {
            @Override
    ...
    private void animationUserProfileHeader(ProfileHeaderViewHolder viewHolder)
    {
        if (!lockedAnimations)
        {
            ...
            viewHolder.ivUserProfilePhoto.setTranslationY(-viewHolder.ivUserProfilePhoto
                    .getHeight());
            viewHolder.vUserDetails.setTranslationY(-viewHolder.vUserDetails.getHeight());
            viewHolder.vUserStats.setAlpha(0);

            viewHolder.vUserProfileRoot.animate().translationY(0).setDuration(300)
                    .setInterpolator(INTERPOLATOR);
            viewHolder.ivUserProfilePhoto.animate().translationY(0).setDuration(300)
                    .setStartDelay(100).setInterpolator(INTERPOLATOR);
            viewHolder.vUserDetails.animate().translationY(0).setDuration(300).setStartDelay(200)
                    .setInterpolator(INTERPOLATOR);
            viewHolder.vUserStats.animate().alpha(1).setDuration(200).setStartDelay(400)
    ...
    private void animateUserProfileOptions(ProfileOptionsViewHolder viewHolder)
    {
        if (!lockedAnimations)
        {
            ...
            viewHolder.vButtons.animate().translationY(0).setDuration(300).setStartDelay
                    (USE_OPTIONS_ANIMATION_DELAY).setInterpolator(INTERPOLATOR);
            viewHolder.vUnderline.animate().scaleX(1).setDuration(200).setStartDelay
                    (USE_OPTIONS_ANIMATION_DELAY + 300).setInterpolator(INTERPOLATOR).start();
        }
    }
    ...
    private void animatePhoto(PhotoViewHolder viewHolder)
    {
        if (!lockedAnimations)
        {
            ...
            if (lastAnimationItem == viewHolder.getPosition())
            {
                setLockedAnimations(true);
            }

            long animationDelay = profileHeaderAnimationStartTime + MAX_PHOTO_ANIMATION_DELAY -
            ...
            viewHolder.flRoot.animate().scaleX(1).scaleY(1).setDuration(200).setStartDelay
